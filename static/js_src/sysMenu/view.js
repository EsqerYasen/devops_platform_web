  ...
  var menuTemplate = require('./templates/menu.ejs');
  ...
  var Screen = Backbone.View.extend({
    ...
    showMenu: function(limit, offset) {
      this.model.getUserMenus().then(function(res) {
        ...
        if (res && res.length) {
          $.each(res, function(i, item){
            if(item.parent_id === -1){
              ...
              menuList["menu_"+item.id] = item.has_sub_menu ? $.extend({submenu: []}, item): item;
            } else {
              menusUnhandled.push(item.has_sub_menu ? $.extend({submenu: []}, item): item)
            }
          });
          var found, idx;
          var looping = menusUnhandled.length;
            $.each(menusUnhandled, function(i, item){
              found = this.getMenuById(item.parent_id, menuList);
              if (found) {
        ...
        $('#side-menu').append(menuTemplate({items:Object.values(menuList)}));
        this.listener();
    ...
    getMenuById: function(id, menu){
      console.log(new Date());
      if (typeof menu['menu_'+id] !== 'undefined'){
        return menu['menu_'+id]
      }
      return false;
      // var keys = this.getMenuLoopingKeys(menu);
      // var found = false;
      // if (keys && keys.length){
      ...
      //         found = menu[key];
      //       } else {
      //         if (menu[key].submenu.length>0) {
      //           return this.getMenuById(id, menu[key].submenu);
      //         }
      //       }
      //     }
      //   }.bind(this));
    ...
    getMenuLoopingKeys: function(menuItem){
      console.log(new Date(), menuItem);
